...
if not pending_files:
    log("✨ No pending videos found! Queue is empty.")
    exit(0)

log(f"🚀 Found {len(pending_files)} videos to process.")
...
for idx, file_record in enumerate(pending_files):
    ...
    # Cleanup old temp files before starting
    ...
    try:
        ...
        # 🚀 Handle corrupted or unreadable video streams safely (Dummy Files)
        raw_duration = probe_result.stdout.strip()
        if not raw_duration:
             raise Exception(f"FFprobe could not read the stream. (File might be corrupted or a dummy file. Size: {file_size} bytes)")
             
        duration = float(raw_duration)
        log(f"   ⏱️ Duration: {duration}s")

        # C. Short Video Logic (Updated to 120 seconds)
        ...
        if duration < 120:
            ...
            if not os.path.exists(poster_file) or os.path.getsize(poster_file) < 1024:
                raise Exception("Thumbnail generation failed (file empty).")
        ...
        ffmpeg_preview = [
            # 🚀 Suppress HEVC/x265 decoder panics and ignore frame errors
        ...
        # 🚀 THE NEW INTEGRITY CHECK: Ensure FFmpeg didn't output a broken file due to a network drop
        if not os.path.exists(preview_file) or os.path.getsize(preview_file) < 5000:
            raise Exception("Preview generation failed or stream dropped (file too small).")

        # E. Upload & Update
        log("   ☁️ Pushing to Edge...")
